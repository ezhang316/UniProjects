...
reveal_grid__body:
        ...
        # convert this C function to MIPS [subset 0]
...
reveal_grid_body_loop0:
        bge     $t0, N_ROWS, reveal_grid__epilogue              # while (row < N_ROWS) {
        
        li      $t1, 0                                          # int col = 0; $t1
reveal_grid_body_loop1:
        bge     $t1, N_COLS, reveal_grid_body_end1              # while (col < N_COLS) {
        
        mul     $t2, $t0, N_COLS                                # calculates address of start of row
        add     $t3, $t2, $t1                                   # adds which column onto row address
        la      $t4, grid                                       #   
        add     $t4, $t4, $t3                                   # finds &grid[row][col] puts into $t4
        
        li      $t2, 0
        or      $t3, $t2, IS_RVLD_MASK                          # grid[row][col]|= IS_RVLD_MASK;
        sb      $t3, ($t4)                                      # storing grid[row][col]
...
place_bombs__prologue:
        ...
        addiu   $sp, $sp, -4     # stores $s0
        sw      $s0, ($sp)
        addiu   $sp, $sp, -4     # stores $s1
        sw      $s1, ($sp)
        addiu   $sp, $sp, -4     # stores $s2
        sw      $s2, ($sp)
        addiu   $sp, $sp, -4     # stores $s3
        sw      $s3, ($sp)

place_bombs__body:

        # convert this C function to MIPS
        ...
        lw      $s0, bomb_count     # int x = total_bombs; $s0
        li      $s1, 0              # int i = 0;  $s1
        move    $s2, $a0            # stores bad_row in $s2
        move    $s3, $a1            # stores bad_col in $s3

place_bombs_body_loop0:
        bge     $s1, $s0, place_bombs_body_end0 # while (i < total_bombs) {
        
        move    $a0, $s2            # puts bad_row in $a0
        move    $a1, $s3            # puts bad_col in $a1
        jal     place_single_bomb   # place_single_bomb(bad_row, bad_col);
        
        addi    $s1, $s1, 1                     # i++;
        j       place_bombs_body_loop0          # }
        
place_bombs_body_end0:


place_bombs__epilogue:
        lw      $s3, 0($sp)     # restores $s3
        addiu   $sp, $sp, 4
        lw      $s2, 0($sp)     # restores $s2
        addiu   $sp, $sp, 4
...
mark_cell__body:
        ...
        mul  $t2, $a0, N_COLS      #   calculates address of start of row
        add  $t3, $t2, $a1         #   adds which column onto row address
        la   $t4, grid             #   
        add  $t4, $t4, $t3         #   finds &grid[row][col]; puts into $t4
        lb   $t5, ($t4)            #   loads grid[row][col] into $t5
        
        li   $t0, IS_RVLD_MASK
        and  $t1, $t0, $t5
        bne  $t0, $t1, mark_cell_if_RVLD_false     # if (grid[row][col] & IS_RVLD_MASK) {
        
mark_cell_if_RVLD_true:

        lw      $t0, debug_mode
        li      $t1, 1
        bne     $t0, $t1, mark_cell_if_debug_false        # if (debug mode) {
        
mark_cell_if_debug_true:
        j       mark_cell__epilogue       # return }
        
mark_cell_if_debug_false:
        la   $a0, mark_error      # printf("Cannot mark a revealed cell.\n");
        li   $v0, 4
        syscall
        j       mark_cell__epilogue     # return }
        
mark_cell_if_RVLD_false:
        li   $t0, IS_MRKD_MASK
        and  $t1, $t0, $t5
        bne  $t0, $t1, mark_cell_if_MRKD_false    # if (grid[row][col] & IS_MRKD_MASK) {
        
mark_cell_if_MRKD_true:
        not  $t0, $t0                     # IS_MRKD_MASK = ~IS_MRKD_MASK
        and  $t5, $t5, $t0                     # grid[row][col] &= IS_MRKD_MASK
        sb   $t5, ($t4)                   # storing new grid[row][col]
...
mark_cell_if_MRKD_false:                      # else {
        li   $t0, IS_MRKD_MASK            # loads IS_MRKD_MASK
        or   $t5, $t5, $t0                     # grid[row][col] |= IS_MRKD_MASK
        sb   $t5, ($t4)                   # storing new grid[row][col]
...
reveal_cell__body:
        ...
        li   $t0, IS_MRKD_MASK
        and  $t1, $t0, $t5
        bne  $t0, $t1, reveal_cell_if_MRKD_false         # if (grid[row][col] & IS_MRKD_MASK) {
        
reveal_cell_if_MRKD_true:

        lw      $t0, debug_mode
        li      $t1, 1
        bne     $t0, $t1, reveal_cell_if_debug_false0    # if (debug mode) {
        
reveal_cell_if_debug_true0:
        j       reveal_cell__epilogue                    # return
                                                         # }
reveal_cell_if_debug_false0:
        la   $a0, reveal_error                           # printf("Cannot reveal a marked cell.\n");
        li   $v0, 4
        syscall
        j       reveal_cell__epilogue                    # return }
        
reveal_cell_if_MRKD_false:
        li   $t0, IS_RVLD_MASK
        and  $t1, $t0, $t5
        bne  $t0, $t1, reveal_cell_if_RVLD_false         # if (grid[row][col] & IS_RVLD_MASK) {
        
reveal_cell_if_RVLD_true:
        lw      $t0, debug_mode
        li      $t1, 1
        bne     $t0, $t1, reveal_cell_if_debug_false1    # if (debug mode) {
        
reveal_cell_if_debug_true1:
        j       reveal_cell__epilogue                    # return
                                                         # }
reveal_cell_if_debug_false1:
        la   $a0, already_revealed                       # printf("Cell is already revealed.\n");
        li   $v0, 4
        syscall
        j       reveal_cell__epilogue                    # return
                                                         # }
reveal_cell_if_RVLD_false:                    
        li   $t0, IS_BOMB_MASK
        and  $t1, $t0, $t5
        bne  $t0, $t1, reveal_cell_if_BOMB_false         # if (grid[row][col] & IS_BOMB_MASK) {
        
reveal_cell_if_BOMB_true:
        li   $t0, LOSE
        sw   $t0, game_state                             # game_state = LOSE
        
reveal_cell_if_BOMB_false:                               # }
        li   $t0, VALUE_MASK
        and  $t1, $t0, $t5
        move $s0, $t4                                    #    stores &grid[row][col] in $s0
        move $s1, $t5                                    #    stores grid[row][col] in $s1
        bnez $t1, reveal_cell_if_VALUE_false             # if (grid[row][col] & VALUE_MASK==0) {

reveal_cell_if_VALUE_true:
        jal  clear_surroundings                          # clear_surroundings(row, col);
        
        j    reveal_cell_if_LOSE_TRUE
        
reveal_cell_if_VALUE_false:                              #  }
        li   $t0, IS_RVLD_MASK                           # loads IS_RVLD_MASK
        or   $s1, $s1, $t0                               # grid[row][col] |= IS_RVLD_MASK
        sb   $s1, ($s0)                                  # storing grid[row][col]
        
        lw   $t0, game_state
        li   $t1, LOSE
        beq  $t0, $t1, reveal_cell_if_LOSE_TRUE          # if (game_state != LOSE) {
        
reveal_cell_if_LOSE_FALSE:
        lw   $t0, cells_left
        addi $t0, -1
        sw   $t0, cells_left                             # cells_left--; }
        
reveal_cell_if_LOSE_TRUE:                                # }

        lw   $t0, cells_left
        bnez $t0, reveal_cell_if_WIN_FALSE               #   if (cells_left == 0) {
        
reveal_cell_if_WIN_TRUE:
        li   $t0, WIN
        sw   $t0, game_state                             # game_state = WIN     
        
reveal_cell_if_WIN_FALSE:                                # }
...
clear_surroundings__body:
        ...
        blt     $a0, $t0, clear_surroundings__epilogue    #   if (row < 0 || row >= N_ROWS || col < 0 || col >= N_COLS) {
        blt     $a1, $t0, clear_surroundings__epilogue    #     return;
        bge     $a0, $t1, clear_surroundings__epilogue    #   }
        bge     $a1, $t2, clear_surroundings__epilogue
        ...
        li      $t0, IS_RVLD_MASK
        and     $t1, $t0, $t5
        bne     $t0, $t1, clear_surroundings_if_RVLD_false       # if (grid[row][col] & IS_RVLD_MASK) {
        
clear_surroundings_if_RVLD_true: 
        j       clear_surroundings__epilogue                     #   return;
                                                                 # }
clear_surroundings_if_RVLD_false:                                # // Reveal the cell.
        li   $t0, IS_RVLD_MASK                                   # loads IS_RVLD_MASK
        or   $t5, $t5, $t0                                       # grid[row][col] |= IS_RVLD_MASK
        sb   $t5, ($t4)                                          # storing grid[row][col]
        ...
        li   $t0, VALUE_MASK
        and  $t1, $t0, $t5
        beqz $t1, clear_surroundings_if_VALUE_false              # if (grid[row][col] & VALUE_MASK) {
        
clear_surroundings_if_VALUE_true: 
        j       clear_surroundings__epilogue                     #   return; }

clear_surroundings_if_VALUE_false: 
        
        move $s0, $a0                                            # Stores row in $s0
        move $s1, $a1                                            # Stores col in $s1
                                                                 #   // Recurse to the surrounding cells in the grid.
        addi $a0, $s0, -1
        move $a1, $s1
...
update_highscore__body:
        ...
        lw      $t0, high_score                             # loads high_score.score
        bge     $t0, $a0, update_highscore_if_false0        # if (high_score.score < score) {
        
update_highscore_if_true0:
        sw      $a0, high_score                             # high_score.score = score;

        li      $t0, 0                                      # int i = 0;     $t0
update_highscore_loop0:
        la      $t1, user_name                              # calculating &user_name[i]
        add     $t1, $t1, $t0                               # adds i to &user_name
        lb      $t2, ($t1)                                  # loads user_name[i] into $t2
        beqz    $t2, update_highscore_loop_end0             # while (user_name[i] != '\0') {
        
        la      $t1, high_score                             # calculating &high_score
        addi    $t1, $t1, N_INT_BYTES                       # offsetting to find &high_score.name
        add     $t1, $t1, $t0                               # finding &high_score.name[i]; puts into $t1
        
        sb      $t2, ($t1)                                  # high_score.name[i] = user_name[i];
        addi    $t0, $t0, 1                                 # i++;
                
        b       update_highscore_loop0                      # }
        
update_highscore_loop_end0:
        li      $t2, 0                          # loads null terminator into $t2
        la      $t1, high_score                 # loads &high_score
        addi    $t1, $t1, N_INT_BYTES           # loads &high_score.name
        add     $t1, $t1, $t0                   # finds &high_score.name[i]; puts into $t1
        sb      $t2, ($t1)                      # high_score.name[i] = '\0'
        
        li      $v0, TRUE                       # sets return value as TRUE
        j       update_highscore__epilogue      # return TRUE;

update_highscore_if_false0:
        li      $v0, FALSE      # sets return value as FALSE. return FALSE;
...
print_scores__body:
        ...
        li      $t0, 0                              # int i = 0;       $t0
        li      $t1, MAX_SCORES                     # loads MAX_SCORES $t1
        li      $t2, USER_SCORE_SIZE                # loads USER_SCORE_SIZE $t2
        
print_scores_loop0:
        bge     $t0, $t1, print_scores_loop_end0    #   while(i < MAX_SCORES) {

        la      $t3, scores                         # finds &scores
        mul     $t4, $t2, $t0                       # calculates bytes needed for &scores[i]
        add     $t4, $t4, $t3                       # finds &scores[i]; puts into $t4
        
        lw      $t3, ($t4)                          # loads scores[i].score (or curr.score) into $t3
        li      $t5, -1                             # loads -1 into $t5
        
        bne     $t3, $t5, print_scores_if_false0    #if (curr.score == -1) {
        
print_scores_if_true0:
        
        j       print_scores_loop_end0              # break;
        
print_scores_if_false0:                             # }
        
        la      $a0, scores_line_msg                # printf("------------------------------\n");
        li      $v0, 4
        syscall 

        la      $a0, scores_username_msg            # printf("* USERNAME:\t");
        li      $v0, 4
        syscall         

        addi    $a0, $t4, N_INT_BYTES               # loads &curr.name into $a0
        li      $v0, 4                              # printf("%s", curr.name);
        syscall 
        ...
        la      $a0, scores_score_msg               # printf("* SCORE:\t");
        li      $v0, 4
        syscall         

        move    $a0, $t3                            # loads curr.score into $a0
        li      $v0, 1                              # printf("%d", curr.score);
        syscall 
        ...
        addi    $t0, $t0, 1                         # i++;
        b       print_scores_loop0                  # }
        
print_scores_loop_end0:
